...
public class TabelleStatistikSonstigeVeranstaltung {
    ...
    public int getZusammengerechneteSonstigeMannstunden(int jahr, int veranstaltungKategorie) throws SQLException {
        Statement statement = DatenbankZugriff.getInstance().getDbConnection().createStatement();
        logging.logSQL((Object)("SELECT sum(mannstunden) FROM statistiksonstigeveranstaltung where jahr = " + jahr + " and kategorie = " + veranstaltungKategorie + " and mandantID = " + runApplication.PROPERTIES.get("MandantID") + ";"));
        ResultSet result = statement.executeQuery("SELECT sum(mannstunden) FROM statistiksonstigeveranstaltung where jahr = " + jahr + " and kategorie = " + veranstaltungKategorie + " and mandantID = " + runApplication.PROPERTIES.get("MandantID") + ";");
        if (result.next()) {
