...
public class AusbilderStatistikAO
extends JFrame {
    ...
    public static void start(final int jahr) {
        ...
        Thread threadStatistik = new Thread(){
            ...
            @Override
            public void run() {
                AusbilderStatistikAO ausbilderStatistik = new AusbilderStatistikAO(jahr);
                ausbilderStatistik.setSize(runApplication.widthStatistikGUI, runApplication.higthStatistikGUI);
                ausbilderStatistik.setDefaultCloseOperation(2);
                ausbilderStatistik.add(buttonZurueck);
                ausbilderStatistik.add(buttonPdfExport);
                ausbilderStatistik.add(buttonJpgExport);
                ausbilderStatistik.add(jahresAuswahl);
                ausbilderStatistik.add(buttonNeuLaden);
                RefineryUtilities.centerFrameOnScreen((Window)ausbilderStatistik);
                Image icon = new ImageIcon("images/icon.jpg").getImage();
                ausbilderStatistik.setIconImage(icon);
                if (runApplication.EINSTELLUNGEN.get("AlwaysOnTop").equals("1")) {
                    ausbilderStatistik.setAlwaysOnTop(true);
                }
                ausbilderStatistik.setVisible(true);
    ...
    public AusbilderStatistikAO(int jahr) {
        super("FeuerwehrManagementSystem - Anwesenheitstatistik Dienstabend");
        this.myButtons(jahr);
        JPanel jpanel = AusbilderStatistikAO.createPanel(jahr);
    ...
    public static CategoryDataset createDataset(int jahr) {
        ...
        try {
            ...
            while (i < mitgliederListe.length) {
                int a1 = tabPlan.getCountAusbilder1(mitgliederIDListe[i], jahr);
                int a2 = tabPlan.getCountAusbilder2(mitgliederIDListe[i], jahr);
                int wert = a1 + a2;
    ...
    public static JFreeChart createChart(CategoryDataset categorydataset) {
        JFreeChart jfreechart = ChartFactory.createBarChart((String)" ", (String)"Personen", (String)"Ausbilder Anzahl", (CategoryDataset)categorydataset, (PlotOrientation)PlotOrientation.HORIZONTAL, (boolean)true, (boolean)true, (boolean)false);
    ...
    public static JPanel createPanel(int jahr) {
        JFreeChart jfreechart = AusbilderStatistikAO.createChart(AusbilderStatistikAO.createDataset(jahr));
    ...
    private void myButtons(final int jahr) {
        ...
        buttonNeuLaden.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (jahresAuswahl.getSelectedItem().toString().equals("<bitte w\u00e4hlen>")) {
                    ...
                    AusbilderStatistikAO.this.dispose();
                    ...
                    AusbilderStatistikAO.start(Integer.parseInt(jahresAuswahl.getSelectedItem().toString()));
                }
            }
        });
        ...
        buttonPdfExport.addActionListener(new ActionListener(){
            ...
            @Override
            public void actionPerformed(ActionEvent arg0) {
                AusbilderStatistikAO.this.chooser.setFileSelectionMode(1);
                AusbilderStatistikAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte PDF Export in: " + AusbilderStatistikAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(AusbilderStatistikAO.this.chooser.getSelectedFile().getPath()) + "/AnwesenheitDienstabendListPDF_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".pdf";
                MyChartUtils.writeChartToPDF(AusbilderStatistikAO.createChart(AusbilderStatistikAO.createDataset(jahr)), 550, 800, outputFile);
        ...
        buttonJpgExport.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                AusbilderStatistikAO.this.chooser.setFileSelectionMode(1);
                AusbilderStatistikAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte JPG Export in: " + AusbilderStatistikAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(AusbilderStatistikAO.this.chooser.getSelectedFile().getPath()) + "/AnwesenheitDienstabendListJPG_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".jpg";
                MyChartUtils.writeChartToJPEG(AusbilderStatistikAO.createChart(AusbilderStatistikAO.createDataset(jahr)), 1000, 800, outputFile);
