...
public class EinsatzMannstundenproMonatAO
extends JFrame {
    ...
    public static void start(int jahr) {
        EinsatzMannstundenproMonatAO mannstunden = new EinsatzMannstundenproMonatAO(jahr);
    ...
    public EinsatzMannstundenproMonatAO(int jahr) {
        super("FeuerwehrManagementSystem - Mannstunden Einsatz pro Monat");
        this.myButtons(jahr);
        JPanel jpanel = EinsatzMannstundenproMonatAO.createPanel(jahr);
    ...
    public static CategoryDataset createDataset(int jahr) {
        ...
        try {
            ...
            while (i < monate.length) {
                defaultcategorydataset.addValue((double)statistik.getZusammengerechneteMannstunden(jahr, monate[i]), (Comparable)((Object)s), (Comparable)((Object)monateName[i]));
                ++i;
            }
        }
    ...
    public static JPanel createPanel(int jahr) {
        JFreeChart jfreechart = EinsatzMannstundenproMonatAO.createChart(EinsatzMannstundenproMonatAO.createDataset(jahr));
    ...
    private void myButtons(final int jahr) {
        ...
        buttonPdfExport.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                EinsatzMannstundenproMonatAO.this.chooser.setFileSelectionMode(1);
                EinsatzMannstundenproMonatAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte PDF Export in: " + EinsatzMannstundenproMonatAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(EinsatzMannstundenproMonatAO.this.chooser.getSelectedFile().getPath()) + "/Ausr\u00fcckezeit_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".pdf";
                MyChartUtils.writeChartToPDF(EinsatzMannstundenproMonatAO.createChart(EinsatzMannstundenproMonatAO.createDataset(jahr)), 550, 800, outputFile);
        ...
        buttonJpgExport.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                EinsatzMannstundenproMonatAO.this.chooser.setFileSelectionMode(1);
                EinsatzMannstundenproMonatAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte JPG Export in: " + EinsatzMannstundenproMonatAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(EinsatzMannstundenproMonatAO.this.chooser.getSelectedFile().getPath()) + "/Ausr\u00fcckezeit_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".jpg";
                MyChartUtils.writeChartToJPEG(EinsatzMannstundenproMonatAO.createChart(EinsatzMannstundenproMonatAO.createDataset(jahr)), 1000, 800, outputFile);
