...
public class AnwesenheitEinsatzAO
extends JFrame {
    ...
    public static void start(final int jahr) {
        ...
        Thread threadStatistik = new Thread(){
            ...
            @Override
            public void run() {
                AnwesenheitEinsatzAO anweseneheitStatistik = new AnwesenheitEinsatzAO(jahr);
    ...
    public AnwesenheitEinsatzAO(int jahr) {
        super("FeuerwehrManagementSystem - Anwesenheitstatistik Einsatz");
        this.myButtons(jahr);
        JPanel jpanel = AnwesenheitEinsatzAO.createPanel(jahr);
    ...
    public static CategoryDataset createDataset(int jahr) {
        ...
        try {
            ...
            while (i < tabMitglieder.getMitgliederCountGruppe1()) {
                int wert = tabAnwesend.getBeteiligungByKategorie(tabMitglieder.getIdByGuiString(mitgliederListe[i]), 1, jahr);
    ...
    public static JPanel createPanel(int jahr) {
        JFreeChart jfreechart = AnwesenheitEinsatzAO.createChart(AnwesenheitEinsatzAO.createDataset(jahr));
        logging.logInfo((Object)"Erstelle Statistik panel...");
    ...
    private void myButtons(final int jahr) {
        ...
        buttonNeuLaden.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (jahresAuswahl.getSelectedItem().toString().equals("<bitte w\u00e4hlen>")) {
                    ...
                    AnwesenheitEinsatzAO.this.dispose();
                    ...
                    AnwesenheitEinsatzAO.start(Integer.parseInt(jahresAuswahl.getSelectedItem().toString()));
                }
            }
        });
        ...
        buttonPdfExport.addActionListener(new ActionListener(){
            ...
            @Override
            public void actionPerformed(ActionEvent arg0) {
                AnwesenheitEinsatzAO.this.chooser.setFileSelectionMode(1);
                AnwesenheitEinsatzAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte PDF Export in: " + AnwesenheitEinsatzAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(AnwesenheitEinsatzAO.this.chooser.getSelectedFile().getPath()) + "/AnwesenheitEinsatzListPDF_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".pdf";
                MyChartUtils.writeChartToPDF(AnwesenheitEinsatzAO.createChart(AnwesenheitEinsatzAO.createDataset(jahr)), 550, 800, outputFile);
        ...
        buttonJpgExport.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                AnwesenheitEinsatzAO.this.chooser.setFileSelectionMode(1);
                AnwesenheitEinsatzAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte JPG Export in: " + AnwesenheitEinsatzAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(AnwesenheitEinsatzAO.this.chooser.getSelectedFile().getPath()) + "/AnwesenheitEinsatzListJPG_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".jpg";
                MyChartUtils.writeChartToJPEG(AnwesenheitEinsatzAO.createChart(AnwesenheitEinsatzAO.createDataset(jahr)), 1000, 800, outputFile);
