...
public class AnwesenheitGesamtAO
extends JFrame {
    ...
    public static void start(final int jahr) {
        ...
        Thread threadStatistik = new Thread(){
            ...
            @Override
            public void run() {
                AnwesenheitGesamtAO anweseneheitStatistik = new AnwesenheitGesamtAO(jahr);
    ...
    public AnwesenheitGesamtAO(int jahr) {
        super("FeuerwehrManagementSystem - Anwesenheitstatistik Gesamt");
        this.myButtons(jahr);
        JPanel jpanel = AnwesenheitGesamtAO.createPanel(jahr);
    ...
    public static CategoryDataset createDataset(int jahr) {
        ...
        try {
            ...
            int i = 0;
            while (i < length) {
                int wert = tabAnwesend.getGesamtBeteiligung(tabMitglieder.getIdByGuiString(mitgliederListe[i]), jahr);
    ...
    public static JPanel createPanel(int jahr) {
        JFreeChart jfreechart = AnwesenheitGesamtAO.createChart(AnwesenheitGesamtAO.createDataset(jahr));
    ...
    private void myButtons(final int jahr) {
        ...
        buttonNeuLaden.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (jahresAuswahl.getSelectedItem().toString().equals("<bitte w\u00e4hlen>")) {
                    ...
                    AnwesenheitGesamtAO.this.dispose();
                    ...
                    AnwesenheitGesamtAO.start(Integer.parseInt(jahresAuswahl.getSelectedItem().toString()));
                }
            }
        });
        ...
        buttonPdfExport.addActionListener(new ActionListener(){
            ...
            @Override
            public void actionPerformed(ActionEvent arg0) {
                AnwesenheitGesamtAO.this.chooser.setFileSelectionMode(1);
                AnwesenheitGesamtAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte PDF Export in: " + AnwesenheitGesamtAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(AnwesenheitGesamtAO.this.chooser.getSelectedFile().getPath()) + "/AnwesenheitListPDF_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".pdf";
                MyChartUtils.writeChartToPDF(AnwesenheitGesamtAO.createChart(AnwesenheitGesamtAO.createDataset(jahr)), 550, 800, outputFile);
        ...
        buttonJpgExport.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                AnwesenheitGesamtAO.this.chooser.setFileSelectionMode(1);
                AnwesenheitGesamtAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte JPG Export in: " + AnwesenheitGesamtAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(AnwesenheitGesamtAO.this.chooser.getSelectedFile().getPath()) + "/AnwesenheitListJPG_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".jpg";
                MyChartUtils.writeChartToJPEG(AnwesenheitGesamtAO.createChart(AnwesenheitGesamtAO.createDataset(jahr)), 1000, 800, outputFile);
