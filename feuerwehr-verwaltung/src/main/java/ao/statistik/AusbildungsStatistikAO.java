...
public class AusbildungsStatistikAO
extends JFrame {
    ...
    public static void start(final int jahr) {
        ...
        Thread threadStatistik = new Thread(){
            ...
            @Override
            public void run() {
                AusbildungsStatistikAO ausbildung = new AusbildungsStatistikAO(jahr);
                ausbildung.setSize(runApplication.widthStatistikGUI, runApplication.higthStatistikGUI);
                ausbildung.add(buttonZurueck);
                ausbildung.add(buttonPdfExport);
                ausbildung.add(buttonJpgExport);
                ausbildung.add(jahresAuswahl);
                ausbildung.add(buttonNeuLaden);
                ausbildung.setDefaultCloseOperation(2);
                RefineryUtilities.centerFrameOnScreen((Window)ausbildung);
                Image icon = new ImageIcon("images/icon.jpg").getImage();
                ausbildung.setIconImage(icon);
                if (runApplication.EINSTELLUNGEN.get("AlwaysOnTop").equals("1")) {
                    ausbildung.setAlwaysOnTop(true);
                }
                ausbildung.setVisible(true);
    ...
    public AusbildungsStatistikAO(int jahr) {
        super("FeuerwehrManagementSystem - Ausbildungsstatistik");
        this.myButtons(jahr);
        JPanel chartPanel = AusbildungsStatistikAO.createDemoPanel(jahr);
    ...
    public static CategoryDataset createDataset(int jahr) {
        ...
        try {
            ...
            int[] veranstaltungKategorieID = Utils.listToIntArray(tabKategorie.getAllKategorienID());
            int i = 0;
            while (i < mitgliederListe.length) {
                int g = 0;
                while (g < veranstaltungKategorieID.length) {
                    dataset.addValue((double)tabAusbildung.getBeteiligungByKategorie(mitgliederIDListe[i], veranstaltungKategorieID[g], jahr), (Comparable)((Object)veranstaltungKategorie[g]), (Comparable)((Object)mitgliederListe[i]));
                    ++g;
                }
                ++i;
            }
    ...
    public static JPanel createDemoPanel(int jahr) {
        JFreeChart chart = AusbildungsStatistikAO.createChart(AusbildungsStatistikAO.createDataset(jahr));
    ...
    private void myButtons(final int jahr) {
        ...
        buttonNeuLaden.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (jahresAuswahl.getSelectedItem().toString().equals("<bitte w\u00e4hlen>")) {
                    ...
                    AusbildungsStatistikAO.this.dispose();
                    ...
                    AusbildungsStatistikAO.start(Integer.parseInt(jahresAuswahl.getSelectedItem().toString()));
                }
            }
        });
        ...
        buttonPdfExport.addActionListener(new ActionListener(){
            ...
            @Override
            public void actionPerformed(ActionEvent arg0) {
                AusbildungsStatistikAO.this.chooser.setFileSelectionMode(1);
                AusbildungsStatistikAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte PDF Export in: " + AusbildungsStatistikAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(AusbildungsStatistikAO.this.chooser.getSelectedFile().getPath()) + "/AbwsenheitslistePDF_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".pdf";
                MyChartUtils.writeChartToPDF(AusbildungsStatistikAO.createChart(AusbildungsStatistikAO.createDataset(jahr)), 550, 800, outputFile);
        ...
        buttonJpgExport.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent arg0) {
                AusbildungsStatistikAO.this.chooser.setFileSelectionMode(1);
                AusbildungsStatistikAO.this.chooser.showSaveDialog(null);
                logging.logInfo((Object)("Starte JPG Export in: " + AusbildungsStatistikAO.this.chooser.getSelectedFile().getPath()));
                String outputFile = String.valueOf(AusbildungsStatistikAO.this.chooser.getSelectedFile().getPath()) + "/AbwsenheitslistePDF_Stand_" + SbcUtils.timeStamp((String)"dd.MM.yyyy") + ".jpg";
                MyChartUtils.writeChartToJPEG(AusbildungsStatistikAO.createChart(AusbildungsStatistikAO.createDataset(jahr)), 1000, 800, outputFile);
