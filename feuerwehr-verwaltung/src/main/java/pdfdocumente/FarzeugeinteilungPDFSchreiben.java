    ...
    public void createPdf(String filename) throws DocumentException, IOException, SQLException {
        ...
        try {
            ...
            while (f < fahrzeugListe.length) {
                ...
                int platz = 0;
                while (platz < 9) {
                    if (FahrzeugEinteilungAO.sitzplatz[f][platz].isVisible()) {
                        chunk = new Chunk(FahrzeugEinteilungAO.sitzplatz_label[f][platz].getText(), NORMAL_BOLD);
                        textblock.add((Element)chunk);
                        if (FahrzeugEinteilungAO.sitzplatz[f][platz].getSelectedItem().toString().equals("<bitte w\u00e4hlen>")) {
                            chunk = new Chunk(" -- ", NORMAL_FONT);
                            textblock.add((Element)chunk);
                            textblock.add((Element)Chunk.NEWLINE);
                        } else {
                            chunk = new Chunk(FahrzeugEinteilungAO.sitzplatz[f][platz].getSelectedItem().toString(), NORMAL_FONT);
                            textblock.add((Element)chunk);
                            textblock.add((Element)Chunk.NEWLINE);
                        }
                    }
            ...
            if (!FahrzeugEinteilungAO.textfield.getText().equals("")) {
                ...
                while (i < zeilen\u00dcbrige.length) {
                    chunk = new Chunk(zeilen\u00dcbrige[i], NORMAL_FONT);
                    textblock.add((Element)chunk);
                    textblock.add((Element)Chunk.NEWLINE);
                    ++i;
                }
