...
public class BeteiligungsListePDFSchreiben {
    ...
    public void createPdf(String filename, String mitglied, String jahr, String grafikFile) throws DocumentException, IOException, SQLException {
        ...
        textblock.add((Element)Chunk.NEWLINE);
        textblock.add((Element)Chunk.NEWLINE);
        String[] veranstaltungen = Utils.listToArray(tabVeranstaltung.getAllVeranstaltungEinesJahres(jahr));
        int mID = tabMitglied.getIdByGuiString(mitglied);
        int i = 0;
        while (i < veranstaltungen.length) {
            int vID = tabVeranstaltung.getVeranstaltungID(veranstaltungen[i]);
            int anwesendStatus = tabAnwesend.getAnwesendStatus(mID, vID);
            if (!veranstaltungen[i].startsWith("BSW") || anwesendStatus != 0) {
                chunk = new Chunk(veranstaltungen[i], NORMAL_BOLD);
                textblock.add((Element)chunk);
                textblock.add((Element)Chunk.NEWLINE);
                if (anwesendStatus == 1) {
                    ...
                    textblock.add((Element)chunk);
                    chunk = new Chunk(" (" + tabGrund.getAbwesenheitsGrundbyID(tabAbwesend.getGrundOfAbwesenheitsByUser(mID, vID)) + ")", NORMAL_FONT);
                    textblock.add((Element)chunk);
                    textblock.add((Element)Chunk.NEWLINE);
                    textblock.add((Element)Chunk.NEWLINE);
                }
