...
public class PDFHydrantenverzeichnis {
    private static final Font EINLEITUNG = new Font(Font.FontFamily.HELVETICA, 36.0f, 1);
    private static final Font UEBERSCHRIFT_STRASSE = new Font(Font.FontFamily.HELVETICA, 14.0f, 4);
    ...
    public void createPdf(String filename) throws DocumentException, IOException, SQLException {
        ...
        hydrant.add((Element)Chunk.NEWLINE);
        hydrant.add((Element)Chunk.NEWLINE);
        hydrant.add((Element)Chunk.NEWLINE);
        hydrant.add((Element)Chunk.NEWLINE);
        hydrant.add((Element)Chunk.NEWLINE);
        ...
        hydrant.add((Element)Chunk.NEWLINE);
        hydrant.add((Element)Chunk.NEWLINE);
        hydrant.add((Element)einleitung);
        document.add((Element)new Paragraph(hydrant));
        document.newPage();
        ...
        while (i < gesamt) {
            document.add((Element)imgBanner);
            Phrase h = new Phrase();
            Chunk hblock = new Chunk(String.valueOf(tabStrassen.getStrassenName(i)) + ":", UEBERSCHRIFT_STRASSE);
            h.add((Element)Chunk.NEWLINE);
            h.add((Element)Chunk.NEWLINE);
            h.add((Element)hblock);
            h.add((Element)Chunk.NEWLINE);
            h.add((Element)Chunk.NEWLINE);
            hblock = new Chunk(service.getHydrantListe(tabStrassen.getStrassenName(i)), NORMAL_FONT);
            h.add((Element)hblock);
            document.add((Element)new Paragraph(h));
            document.newPage();
