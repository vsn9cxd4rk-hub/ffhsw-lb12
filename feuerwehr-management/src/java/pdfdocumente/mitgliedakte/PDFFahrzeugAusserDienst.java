...
public class PDFFahrzeugAusserDienst {
   ...
   public void createPdf(String filename, String fahrzeug) throws DocumentException, IOException, SQLException {
      ...
      Chunk address = new Chunk("Name: " + fahrzeug, UEBERSCHRIFT);
      personal.add(Chunk.NEWLINE);
      personal.add(Chunk.NEWLINE);
      personal.add(address);
      personal.add(Chunk.NEWLINE);
      address = new Chunk("Das Fahrzaug " + fahrzeug + " wurde mit dem heutigen Tage Außer Dienst gestellt.", NORMAL_FONT);
      personal.add(Chunk.NEWLINE);
      personal.add(address);
      document.add(new Paragraph(personal));
      document.addTitle("FeuerwehrManagementSystem - FahrzuegInfo");
      document.addAuthor("FeuerwehrManagementSystem");
      document.addCreationDate();
      document.addSubject("FahrzuegInfo");
      document.close();
      writer.close();
   }
   ...
   public static void PDFdocumentErstellen(String dateiname, String farhzeug) throws DocumentException, IOException {
      PDFFahrzeugAusserDienst ps = new PDFFahrzeugAusserDienst();

      try {
         ps.createPdf(dateiname, farhzeug);
         ps.printMeasures();
