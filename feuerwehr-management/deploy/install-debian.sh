...
# Ziel: Ubuntu 22.04 LTS / Debian 12 (ohne Docker)
...
# Root-Prüfung
# --------------------------------------------------------------------------
[[ $EUID -ne 0 ]] && error "Dieses Script muss als root ausgeführt werden: sudo bash install.sh"

echo ""
echo "=================================================================="
echo "  Feuerwehr Management System - Installation"
echo "=================================================================="
echo ""
...
read -p "MySQL root Passwort (wird für die Installation benötigt): " MYSQL_ROOT_PASS
[[ -z "$MYSQL_ROOT_PASS" ]] && error "MySQL root Passwort ist erforderlich"
...
apt-get install -y -qq curl wget gnupg2 ca-certificates lsb-release software-properties-common unzip git
...
if ! command -v node &>/dev/null || [[ "$(node -v | cut -d'v' -f2 | cut -d'.' -f1)" -lt "$NODE_VERSION" ]]; then
  info "Node.js $NODE_VERSION wird installiert..."
...
if ! command -v mysql &>/dev/null; then
  info "MySQL wird installiert..."
  DEBIAN_FRONTEND=noninteractive apt-get install -y -qq mysql-server
  systemctl start mysql
  systemctl enable mysql
  success "MySQL installiert und gestartet"
else
  success "MySQL bereits installiert"
fi
...
# --------------------------------------------------------------------------
# MySQL Datenbank, Benutzer und Schema einrichten (via init-db.sql)
# --------------------------------------------------------------------------
info "Datenbank wird initialisiert..."

# init-db.sql auf die konfigurierten Werte anpassen
...
# seed.ts setzt den korrekten bcrypt-Passwort-Hash für den Admin-Benutzer
# und ist per INSERT IGNORE / upsert idempotent (bereits vorhandene Daten bleiben)
...
cp -r "$PROJECT_DIR/webapp/frontend/." "$APP_DIR/frontend-src/" 2>/dev/null || true
